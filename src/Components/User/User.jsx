...
const User = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Axios.get('http://127.0.0.1:3000/api/v1/users/getallusers');
  ...
  const handleCreateNewRow = async (values) => {
    try {
      const response = await Axios.post('http://127.0.0.1:3000/api/v1/auth_users/register', values);
  ...
  const handleEditRow = (row) => {
    ...
    console.log(`http://127.0.0.1:3000/api/v1/users/updateusers/${row.uniqueId}`)
  ...
  const handleSaveRowEdits = async ({ exitEditingMode, values }) => {
    try {
      ...
      const response = await Axios.patch(`http://127.0.0.1:3000/api/v1/users/updateusers/${values.uniqueId}`, values);
  ...
  const handleDeleteRow = useCallback(
    async (row) => {
      ...
      try {
        await Axios.delete(`http://127.0.0.1:3000/api/v1/users/deleteusers/${row.uniqueId}`);
