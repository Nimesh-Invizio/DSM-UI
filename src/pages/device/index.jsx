...
function Devices() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Axios.get(`${process.env.REACT_APP_API_BASE_URL}/servers/${serverId}/shops/${selectedShop}/devices`);
  ...
  const handleEditRow = async (row) => {
    try {
      const response = await Axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/devices/single/${serverId}/${row.id}`
      );
  ...
  const handleDeleteRow = async (row) => {
    ...
    try {
      await Axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/devices/delete/${serverId}/${row.id}`
      );
...
export const EditDeviceModal = ({ open, onClose, onSubmit, values }) => {
 ...
 const handleEditSubmit = async () => {
   const res = await Axios.patch(
     `${process.env.REACT_APP_API_BASE_URL}/devices/update/${uniqueId}/${values.id}`,
     editedValues
   );
   onSubmit(res);
   onClose();
